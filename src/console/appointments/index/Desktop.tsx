...
export default function Desktop({
  ...
  return (
    <DesktopWrapper
      ...
      className="flex flex-col space-y-10 items-start justify-start relative"
    >
      <div className="grid grid-cols-2 gap-5 w-full relative max-h-screen overflow-scroll">
        <Appointments date={date} tag={tag} />
        <div className="sticky top-0 flex flex-col items-start justify-start space-y-5 w-full h-fit">
          <Calendar
            date={date}
            setDate={setDate}
...
function Tags({
  ...
  function Tag({ tag }: { tag: AppointmentStatus | null }) {
    return (
      <motion.div
        className={`flex items-center justify-center space-x-2 rounded-md px-2 py-5 lg:py-3 hover:cursor-pointer ${
